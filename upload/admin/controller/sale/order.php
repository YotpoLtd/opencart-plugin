...
class ControllerSaleOrder extends Controller {
	...
	private function send_mail_after_purchase($order_id)
	{
		...
		$map_enabled = true;//$this->config->get('yotpo_send_map');
		if(!isset($app_key) || !isset($secret_token) || $map_enabled == false)
		{
			return NULL;
		}
		...
		if(isset($token))
		{
		    ...
		    foreach ($products as $order_product) {
		      ...
		      $product_data['image'] = HTTPS_IMAGE . $full_product['image'];
	...
	private function grantOauthAccess($app_key, $secret_token)
	{
		...
		$OAuthStorePath = DIR_SYSTEM . 'library'. $_ds .'oauth-php' . $_ds . 'library' . $_ds . 'OAuthStore.php';
	    $OAuthRequesterPath = DIR_SYSTEM .  'library'. $_ds .'oauth-php' . $_ds . 'library' . $_ds . 'OAuthRequester.php';
  	...
  	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_order->editOrder($this->request->get['order_id'], $this->request->post);
	  		...
	  		// SEND MAP
			$this->send_mail_after_purchase($this->request->get['order_id']);
